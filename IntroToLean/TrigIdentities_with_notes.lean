...
/-
If we don't provide arguments to `sin_add`, Lean will try to guess them by finding
the first match for the left-hand side of the equation.
Try changing `sin_add x x` to just `sin_add` above.
In this case it works out fine, but sometimes more control is needed.
